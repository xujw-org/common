...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  sudo rm -rf build/common && git clone -b main --depth 1 https://github.com/bootli/common build/common
...
git clone -b main --depth 1 https://github.com/bootli/build-actions shangyou
...
if [[ "${GIT_REPOSITORY}" =~ (bootli/build-actions|bootli/autobuild) ]]; then
  rm -rf backups
...
curl -fsSL https://raw.githubusercontent.com/bootli/common/main/common.sh -o common.sh
if [[ $? -ne 0 ]]; then
  wget -q https://raw.githubusercontent.com/bootli/common/main/common.sh -O common.sh
fi
...
if [[ -n "${BENDI_VERSION}" ]]; then
  ...
  export GIT_REPOSITORY="bootli/build-actions"
  sudo rm -rf build common.sh
